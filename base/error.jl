...
const DEFAULT_RETRY_DELAY = 0.05

"""
    retry(f, [retry_on]; n=1, max_delay=0.05) -> Function
...
```
"""
function retry(f::Function, retry_on::Function=DEFAULT_RETRY_ON; n=DEFAULT_RETRY_N, max_delay=DEFAULT_RETRY_DELAY)
    (args...) -> begin
        delay = min(DEFAULT_RETRY_DELAY, max_delay)
        for i = 1:n+1
            try
                return f(args...)
            catch e
